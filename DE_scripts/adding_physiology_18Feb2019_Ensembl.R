...
ExpDesign <- data.frame(row.names=cols, physiology=physiology,condition=condition, species = species, clade=clade,species_condition=species_condition)
ExpDesign
# remove F. nottii
#ExpDesign<-ExpDesign[-c(36,37),]
# remove F. zebrinus
#ExpDesign<-ExpDesign[-c(14,15,16,17),]
#ExpDesign <- write.csv(ExpDesign,"LKJ_ExpDesign.csv")
ExpDesign

m1 <- model.matrix(~condition + species_condition,ExpDesign)
colnames(m1)
m1
...
m1 <- m1[,-idx]
m1
#filtered_counts <- filtered_counts[,-c(14,15,16,17,36,37)]
