...
The following is a DE analysis with the freshwater native, *Fundulus parvapinis* killifish to identify candidate genes to be interested in for downstream analysis.
...
files_list = list.files("~/Documents/UCDavis/Whitehead/RNAseq_15killifish/salmon_denovo_by_species/F_parvapinis")
files <- file.path(dir,"F_parvapinis",files_list, "quant.sf")
files
...
```{r, echo=FALSE}
contigs <- read.csv("~/Documents/UCDavis/Whitehead/intermediate_denovo_annotation_files/F_parvapinis_gene_transcript_table.txt")
dim(contigs)
```
...
```{r, echo=FALSE}
gene_names <- read.csv("~/Documents/UCDavis/Whitehead/annotation_gene_names/F_parvapinis_gene_names.csv")
dim(gene_names)
...
conditions = factor(c("BW","BW","BW","FW","FW","FW","TR","TR"))
...
plot(log2(res.1$baseMean), res.1$log2FoldChange, 
     ...
     main="Fundulus parvapinis (BW vs. FW) (padj<0.05)",xlim=c(1,15),pch=20,cex=1)
...
plot(log2(res.2$baseMean), res.2$log2FoldChange, 
     ...
     main="Fundulus parvapinis (transfer vs. FW) (padj<0.05)",xlim=c(1,15),pch=20,cex=1)
abline(h=c(-1,1), col="blue")
```
...
plot(log2(res.3$baseMean), res.3$log2FoldChange, 
     ...
     main="Fundulus parvapinis (transfer vs. BW) (padj<0.05)",xlim=c(1,15),pch=20,cex=1)
abline(h=c(-1,1), col="blue")
```
...
```{r echo=FALSE}
# TR vs. FW samples
counts_TR_FW <- filtered_norm_counts[,c(4:8)]
...
heatmap.2(d, main="Fundulus parvapinis (padj<0.05, log2FC = ±1)", 
...
```{r echo=FALSE}
# TR vs. BW samples
counts_TR_BW <- filtered_norm_counts[,c(1,2,3,7,8)]
length(combined_transfer_BW)
...
write.csv(all_matrix,"~/Documents/UCDavis/Whitehead/RNAseq_15killifish/DE_results/F_parvapinis_DE_all.csv")
TR_BW_all<-all_matrix[all_matrix$GeneID %in% combined_transfer_BW,]
write.csv(TR_BW_all,"~/Documents/UCDavis/Whitehead/RNAseq_15killifish/DE_results/F_parvapinis_DE_TR_BW.csv")
TR_FW_all<-all_matrix[all_matrix$GeneID %in% combined_transfer_FW,]
write.csv(TR_FW_all,"~/Documents/UCDavis/Whitehead/RNAseq_15killifish/DE_results/F_parvapinis_DE_TR_FW.csv")
```
