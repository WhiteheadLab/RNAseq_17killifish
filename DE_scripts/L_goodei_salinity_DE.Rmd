...
The following is a DE analysis with the freshwater native, *Lucania goodei* killifish to identify candidate genes to be interested in for downstream analysis.
...
files_list = list.files("~/Documents/UCDavis/Whitehead/RNAseq_15killifish/salmon_denovo_by_species/L_goodei")
files <- file.path(dir,"L_goodei",files_list, "quant.sf")
files
...
```{r, echo=FALSE}
contigs <- read.csv("~/Documents/UCDavis/Whitehead/intermediate_denovo_annotation_files/L_goodei_gene_transcript_table.txt")
dim(contigs)
```
...
```{r, echo=FALSE}
gene_names <- read.csv("~/Documents/UCDavis/Whitehead/annotation_gene_names/L_goodei_gene_names.csv")
dim(gene_names)
...
plot(log2(res.1$baseMean), res.1$log2FoldChange, 
     ...
     main="Lucania goodei (BW vs. FW) (padj<0.05)",xlim=c(1,15),pch=20,cex=1)
...
plot(log2(res.2$baseMean), res.2$log2FoldChange, 
     ...
     main="Lucania goodei (transfer vs. FW) (padj<0.05)",xlim=c(1,15),pch=20,cex=1)
abline(h=c(-1,1), col="blue")
```
...
plot(log2(res.3$baseMean), res.3$log2FoldChange, 
     ...
     main="Lucania goodei (transfer vs. BW) (padj<0.05)",xlim=c(1,15),pch=20,cex=1)
abline(h=c(-1,1), col="blue")
```
...
heatmap.2(d, main="Lucania goodei (padj<0.05, log2FC = ±1)", 
...
write.csv(all_matrix,"~/Documents/UCDavis/Whitehead/RNAseq_15killifish/DE_results/L_goodei_DE_all.csv")
TR_BW_all<-all_matrix[all_matrix$GeneID %in% combined_transfer_BW,]
write.csv(TR_BW_all,"~/Documents/UCDavis/Whitehead/RNAseq_15killifish/DE_results/L_goodei_DE_TR_BW.csv")
TR_FW_all<-all_matrix[all_matrix$GeneID %in% combined_transfer_FW,]
write.csv(TR_BW_all,"~/Documents/UCDavis/Whitehead/RNAseq_15killifish/DE_results/L_goodei_DE_TR_FW.csv")
```
