...
---
title: "RNAseq, DE analysis for L_goodei killifish osmotic challenge"
...
design <- design[ , startsWith(names(design),"L_goodei")]
counts <- counts[ , startsWith(names(counts),"L_goodei")]
dim(counts)
...
plot(log2(res$baseMean), res$log2FoldChange, 
     ...
     main="L_goodei (15_ppt vs. 0.2_ppt) (padj<0.05)",xlim=c(1,15),pch=20,cex=1)
...
plot(log2(res$baseMean), res$log2FoldChange, 
     col=ifelse(res$padj < 0.05, "red","gray67"),
     main="L_goodei (transfer vs. 0.2_ppt) (padj<0.05)",xlim=c(1,15),pch=20,cex=1)
abline(h=c(-1,1), col="blue")
resSig = res_ordered[rownames(res_ordered) %in% protein_id, ]
dim(resSig)
genes<-rownames(resSig)
...
plot(log2(res$baseMean), res$log2FoldChange, 
     col=ifelse(res$padj < 0.05, "red","gray67"),
     main="L_goodei (transfer vs. 15_ppt) (padj<0.05)",xlim=c(1,15),pch=20,cex=1)
abline(h=c(-1,1), col="blue")
resSig = res_ordered[rownames(res_ordered) %in% protein_id, ]
dim(resSig)
genes<-rownames(resSig)
...
write.csv(counts_table_ann,"/Users/johnsolk/Documents/UCDavis/Whitehead/counts_stats_byspecies/L_goodei_stats_annotations_counts.csv")
```
...
heatmap.2(d, main="L_goodei, padj<0.05",
