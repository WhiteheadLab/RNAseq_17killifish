...
The following is a DE analysis with *Adenia xenica* to identify candidate genes for downstream comparative gene expression analysis.
...
```{r, echo=FALSE}
contigs <- read.csv("~/Documents/UCDavis/Whitehead/trinity_contig_gene_tables/A_xenica_trinity_contig_gene_table.txt")
dim(contigs)
```
...
```{r, echo=FALSE}
gene_names <- read.csv("~/Documents/UCDavis/Whitehead/annotation_files_test/A_xenica_gene_names.csv")
head(gene_names)
colnames(gene_names)
dim(gene_names)
gene_names_id <- gene_names[,c(2,5)]
